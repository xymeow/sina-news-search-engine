...
%>
<p style="font-size: 18px;margin-bottom: 1px"><a href="<%=results.urls.get(i)%>"><%=results.titles.get(i)%></a></p>
<p
        style="color: #556b2f;font-size: small;margin-top: 1px" ><%=results.urls.get(i)%>
</p>
<p style="width: 700px;line-height: 120%"><%=results.contents.get(i)%></p>
...
<ul class="pagination" style="margin-left: 20%;">
  <% currentPage --;
    if (currentPage >= 1){
   %>
  <li><a href="search2.jsp?currentPage=<%=currentPage%>">&laquo;</a> </li>
  <%}%>
  ...
  <%
    ...
    if (end - begin == pageSize && currentPage < 9){
      currentPage += 2;%>
